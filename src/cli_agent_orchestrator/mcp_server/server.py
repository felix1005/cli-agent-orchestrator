...
def _create_terminal(
    ...
    if current_terminal_id:
        ...
        response = requests.post(
            ...
            params=params,
            # Not MCP_REQUEST_TIMEOUT — this blocks server-side on the new
            # worker terminal's full agent startup (up to ~90s), same as the
            # top-level session-creation call in cli/commands/launch.py.
            timeout=TERMINAL_CREATE_REQUEST_TIMEOUT,
        )
        response.raise_for_status()
        ...
        response = requests.post(
            f"{API_BASE_URL}/sessions", params=params, timeout=TERMINAL_CREATE_REQUEST_TIMEOUT
        )
        response.raise_for_status()
        terminal = response.json()
